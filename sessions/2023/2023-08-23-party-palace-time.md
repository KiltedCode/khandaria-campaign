...
`Qa'dat` and `Caedric` notice one is missing  
...
`Faleor` and `Caedric` can tell nobles are made as they were carrying hidden weapons (not well hidden)  
...
`Caedric` talks about tales but also his restaurant  
...
`Caedric`'s was never large enough to take something like that on  
...
`Faleor` moves towards the Khan, tells `Caedric` "it is gone" and points to head  
